...
def test_large_fixture_generates_workflow_and_liquibase_artifacts_deterministically(
    tmp_path: Path,
    test_config_path: Path,
) -> None:
    ...
    assert all(
        generated[name].startswith("-- Generated by table-factory")
        for index, name in enumerate(expected_names)
        if index != 3
    )
    assert generated[expected_names[3]].startswith("--liquibase formatted sql\n")
    ...
    greenplum_create = generated[expected_names[4]]
    assert (
    ...
    greenplum_insert = generated[expected_names[5]]
    assert (
    ...
    workflow_sql = "\n".join(
        generated[name] for index, name in enumerate(expected_names) if index != 3
    )
    assert "SELECT *" not in workflow_sql.upper()
    for legacy_command in (
        ...
        assert re.search(legacy_command, workflow_sql, flags=re.IGNORECASE) is None
    assert re.search(r"\bDROP\b", workflow_sql, flags=re.IGNORECASE) is None
    liquibase = generated[expected_names[3]]
    assert liquibase.count("DROP EXTERNAL TABLE IF EXISTS") == 1
    assert 'DROP EXTERNAL TABLE IF EXISTS "ext"."replica_large_table_ext" CASCADE;' in liquibase
    payload = _liquibase_payload(liquibase)
    columns = payload["columns"]
    assert isinstance(columns, list)
    assert tuple(column["name"] for column in columns) == EXPECTED_LARGE_COLUMN_NAMES
    assert payload["source_table"] == "original_hive_database.large_table"
...
def test_comments_and_partition_columns_are_flattened_and_escaped(
    ...
    greenplum_external = generated[expected_names[2]]
    greenplum_liquibase = generated[expected_names[3]]
    greenplum_create = generated[expected_names[4]]
    greenplum_insert = generated[expected_names[5]]
    ...
    liquibase_payload = _liquibase_payload(greenplum_liquibase)
    assert liquibase_payload["source_table"] == "original_hive_database.events"
    assert liquibase_payload["columns"] == [
        {"name": "event_id", "type": "int8", "description": "Идентификатор"},
        {"name": "owner", "type": "text", "description": "O'Brien"},
        {"name": "amount", "type": "numeric", "description": "-"},
        {"name": "event_day", "type": "date", "description": "День события"},
    ]
...
def test_recursive_multiple_input_files_generate_twelve_artifacts(
    tmp_path: Path,
    test_config_path: Path,
) -> None:
    input_directory = tmp_path / "input"
    ...
    assert "Generated 12 SQL files" in result.stdout
...
def test_backtick_source_names_work_with_default_target_templates(
    ...
    assert f'CREATE TABLE "dwh"."replica_{table_name}"' in generated[_artifact_names(stem)[4]]
